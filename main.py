...
class Config:
    ...
    ZIP_TIMEOUT = 3600  # 1 hour timeout for zip operations
    ...
    for attempt in range(max_retries):
        try:
                ...
                try:
                    with pyzipper.AESZipFile(
                        ...
                        for file_info in files:
                            ...
                            logger.info(f"Added {arcname} to zip part")
            ...
            success = await asyncio.wait_for(
                loop.run_in_executor(zip_executor, zip_creation_task),
                timeout=Config.ZIP_TIMEOUT
            )
        ...
        try:
            async with upload_semaphore:
                if attempt > 0:
                    ...
                    logger.info(f"Retry {attempt + 1}/{max_retries} after {wait_time:.1f} seconds")
                    await asyncio.sleep(wait_time)
...
async def start(client, message: Message):
    ...
    welcome_text = (
        "🤖 **ربات فشرده‌ساز و آپلودر پیشرفته**\n\n"
        "📦 **امکانات:**\n"
        "• فشرده‌سازی فایل‌ها با رمزگذاری AES-256\n"
        "• تقسیم خودکار به پارت‌های زیر 2GB\n"
        "• مدیریت هوشمند محدودیت‌های تلگرام\n"
        "• پشتیبانی از فایل‌های بزرگ تا 8GB\n\n"
        "🚀 **برای شروع:**\n"
        "1. فایل‌های خود را ارسال کنید\n"
        "2. از /zip برای شروع فرآیند استفاده کنید\n"
        "3. رمز و نام فایل را تنظیم کنید\n"
        "4. منتظر بمانید تا ربات کار را انجام دهد\n\n"
        "⚙️ **دستورات:**\n"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for part_index, part_files in enumerate(parts):
                ...
                # استفاده از تابع فشرده سازی پیشرفته
