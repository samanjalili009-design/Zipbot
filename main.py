...
async def run_bot():
    ...
    try:
        ...
        # ثبت هندلرها با استفاده از MessageHandler و اولویت‌های مشخص
        app.add_handler(MessageHandler(start_handler, filters.command("start")), 0)
        app.add_handler(MessageHandler(start_zip_handler, filters.command("zip")), 1)
        app.add_handler(MessageHandler(start_zip_now_handler, filters.command("zipnow")), 2)
        app.add_handler(MessageHandler(cancel_zip_handler, filters.command("cancel")), 3)
        app.add_handler(MessageHandler(handle_done_command_handler, filters.command("done")), 4)
        
        # هندلر فایل‌ها با اولویت بالاتر
        app.add_handler(MessageHandler(handle_file_handler, filters.document | filters.video | filters.audio | filters.photo), 5)
        
        # هندلر متن با اولویت پایین‌تر
        app.add_handler(MessageHandler(handle_text_message_handler, filters.text & filters.private), 6)
        
        # هندلر callback query
        app.add_handler(CallbackQueryHandler(handle_callback_query_handler), 7)
