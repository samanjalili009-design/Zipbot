...
async def send_msg(app, chat_id, text, reply_id=None):
    try:
...
async def download_with_progress(app, message, file_path, progress):
...
async def upload_with_progress(app, file_path, chat_id, caption, reply_id, progress):
    """آپلود با نمایش پیشرفت"""
    try:
        file_size = os.path.getsize(file_path)
        
        async def progress_callback(current, total):
            await progress.update(current, file_size, "📤 در حال آپلود")
...
async def process_zip_files(app, user_id, chat_id, message_id):
    ...
    try:
        processing_msg = await send_msg(app, chat_id, "📥 در حال دانلود فایل‌ها...", message_id)
        ...
        for file_data in user_files[user_id]:
            try:
                file_msg = await app.get_messages(file_data["chat_id"], file_data["message_id"])
                if not file_msg:
                    continue
                ...
                if await download_with_progress(app, file_msg, file_path, progress):
                    file_size = os.path.getsize(file_path)
                    file_infos.append({
                        'path': file_path,
...
async def main():
    ...
    async def start_handler(client, message: Message):
        if not is_user_allowed(message.from_user.id):
            return
        await send_msg(app, message.chat.id, 
    ...
    async def start_zip_handler(client, message: Message):
        ...
        if user_id not in user_files or not user_files[user_id]:
            await send_msg(app, message.chat.id, "❌ هیچ فایلی برای زیپ کردن وجود ندارد", message.id)
            return
    ...
    async def skip_password_handler(client, message: Message):
        ...
        if user_states.get(user_id) == "waiting_password":
            ...
            await process_zip_files(app, user_id, message.chat.id, message.id)
    ...
    async def clear_files_handler(client, message: Message):
        ...
        await send_msg(app, message.chat.id, "✅ فایل‌ها پاک شدند", message.id)
    ...
    async def status_handler(client, message: Message):
        ...
        await send_msg(app, message.chat.id, status_text, message.id)
    ...
    async def handle_text(client, message: Message):
        ...
        if user_states.get(user_id) == "waiting_password":
            ...
            if len(password) < 4:
                await send_msg(app, message.chat.id, "❌ رمز عبور باید حداقل 4 کاراکتر باشد", message.id)
                return
