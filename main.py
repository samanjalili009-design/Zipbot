...
async def run_bot():
    ...
    try:
        ...
        # دریافت اطلاعات ربات
        me = await app.get_me()
        logger.info(f"🤖 ربات اجرا شد به نام: {me.first_name}")
        logger.info(f"🆔 آیدی ربات: {me.id}")
        
        # ارسال پیام راه‌اندازی
        try:
            await app.send_message(ALLOWED_USER_ID, "✅ ربات زیپ‌ساز با موفقیت راه‌اندازی شد!")
        except:
            pass
