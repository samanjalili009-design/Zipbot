...
DOWNLOAD_CHUNK_SIZE = 131072  # 128KB - افزایش اندازه chunk
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            while True:
                ...
                # وقفه کوتاه برای جلوگیری از block شدن
    ...
    try:
        ...
        with pyzipper.AESZipFile(zip_path, "w", compression=pyzipper.ZIP_DEFLATED, 
            ...
            for i, file_info in enumerate(files, 1):
                ...
                # آپدیت پیشرفت هر 5 فایل یا بیشتر
                if i % 5 == 0 or i == total_files:
                    progress_text = f"⏳ در حال فشرده سازی... {i}/{total_files}"
                    try:
                        await processing_msg.edit_text(progress_text)
                        await asyncio.sleep(0.5)  # وقفه برای جلوگیری از FloodWait
                    except FloodWait as e:
                        await asyncio.sleep(e.value)
                    except Exception:
                        pass
                
                # وقفه برای جلوگیری از overload
                await asyncio.sleep(0.1)
...
async def download_file_with_progress(client, message, file_path, processing_msg, file_size):
    """دانلود فایل با پیشرفت و مدیریت بهتر"""
    ...
    def progress(current, total):
        ...
        # فقط هر 2 ثانیه آپدیت کن تا از FloodWait جلوگیری شود
...
async def process_zip(client, message):
    ...
    if user_id in waiting_for_filename and waiting_for_filename[user_id]:
        ...
        zip_password = zip_password_storage.get(user_id, "1234")  # رمز پیش‌فرض
        
        try:
            # ایجاد task جداگانه برای پردازش سنگین
            asyncio.create_task(
